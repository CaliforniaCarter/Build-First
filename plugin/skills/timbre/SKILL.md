...
Use `tb …` — it's installed globally, so it works from any directory. Prefer
...
feeds drafts automatically). Then run `tb onboard --json` — the only AI
...
stat, a file, a real metric; if you didn't see a receipt, don't write one). Then run `tb
...
**3. Draft two options.** Run `tb post --provider anthropic --json`. It returns **two**
...
**4. They pick.** Run `tb pick --option <0|1> --why "<their reason>"
...
or after a good run of picks), run `tb learn --check`; if anything's pending, ask *"fold
today's N picks into your voice?"* and fold **only on yes** (`tb learn` → updates
...
- `tb posts --json` — their saved posts (`status` draft|posted), plus the **authoritative**
...
- When they tell you they actually posted one, celebrate it and run `tb publish <slug>` to
