...
3. **Probe the gaps (never invent).** Run `tb gaps --json`. For each gap (a real number,
...
4. **Draft two options.** Run `tb post --provider anthropic --json` (drop `--provider
...
5. **They pick.** Ask: **"which feels more like you — 1 or 2? (and why — optional)"** Then run
   `tb pick --option <0|1> --why "<their reason>" --provider anthropic --json`. This
